function Operator(el) {
  ...
  this.commands.dat = function(p, option) {
    ...
    DatArchive.resolveName(path)
      ...
      .catch(function(e) {
        console.error("Error when resolving added portal in operator.js", e);
      });
  };
  ...
  this.commands.fix_port = function() {
    var promises = r.portal.data.port.map(function(portal) {
      return new Promise(function(resolve, reject) {
        ...
        if (r.portal.data.dat == portal) {
          return;
        }
        // resolve dns shortnames to their actual dat:// URIs
        DatArchive.resolveName(portal)
          .then(function(result) {
            result = "dat://" + result + "/";
            resolve(result);
          })
          .catch(function(e) {
            console.error(
              "Error when resolving in fix_port:operator.js",
              e,
              portal
            );
            resolve(portal);
    ...
    Promise.all(promises)
      .then(function(fixed_ports) {
        r.portal.data.port = fixed_ports;
        r.portal.save();
      })
      .catch(function(e) {
        console.error(
          "Error when fixing ports; probably offline or malformed json",
          e
        );
      });
  };
