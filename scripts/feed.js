function Feed(feed_urls) {
  ...
  this.update = async function() {
    this.get_entries();
    setTimeout(function() {
      r.portal.port_list_el.innerHTML = r.feed.get_feed_html();
    }, 1000);
  };
  ...
  this.get_feed_html = function() {
    ...
    for (name in r.feed.portals) {
      ...
      var is_active = last_entry
        ? Math.floor((new Date() - last_entry.timestamp) / 1000)
        : 999999;
      var rune =
        portal.port.indexOf(r.portal.data.dat) > -1 ||
        portal.dat === r.portal.data.dat
          ? "@"
          : "~";
  ...
  this.get_feed = async function(archive) {
    return Promise.all([
      await archive.readFile("portal.json"),
      DatArchive.resolveName(archive.url)
    ])
      .then(values => {
  ...
  this.refresh = function(entries) {
    ...
    for (id in entries) {
      ...
      if (!entry || entry.timestamp > new Date()) {
        continue;
      }
      if (!entry.is_visible()) {
        continue;
    ...
    html +=
      "<div class='entry'><t class='portal'>$rotonde</t><t class='timestamp'>Just now</t><hr/><t class='message' style='font-style:italic'>Welcome to #rotonde, a decentralized social network. Share your dat:// url with others and add theirs into the input bar to get started.</t></div>";
