function Entry(data) {
    ...
    html +=
      ...
      (this.target
        ? "<a href='" +
          this.target +
          "'>" +
          portal_from_hash(this.target.toString()) +
          "</a>"
        : "") +
      "</t>";

    var operation =
      this.portal == r.portal.data.name
        ? "edit:" + this.id + " " + this.message.replace(/\'/g, "&apos;")
        : "quote:" + this.portal + "-" + this.id + " ";
  ...
  this.body = function() {
    ...
    if (this.quote) {
      html +=
        "<t class='message quote' dir='auto'>" +
        this.formatter(this.quote.message) +
        "</t><br/>";
    }
    return html;
  };
